...
for ii = 0, NUMBER_WORKERS do
  ...
  if pid == 0 then 
    ...
    function run_sandbox(sb_env, sb_func, scriptname, ...)
      ...
      pretty.dump(message)
      redis_conn:hset(worker_list_name, worker_name, message)
    ...
    assert(db:connect('localhost:81'))
        ...
        while true do
          posix.sleep(1)
          print ("=================================================")
          print ("Started worker with pid: " .. posix.getpid('pid'))
          print ("=================================================") 
    ...
    -- pid is the process id of the spawned child
    print(posix.getpid('pid') .. ": parent process")
    print("spawned child:" .. pid)

    childPids[ii] = pid
    -- print("printing childPids")
    -- for i,j in pairs(childPids) do print(i,j) end

    if ii == NUMBER_WORKERS then
      print("Chilren spawned, entering supervisor mode")
      pretty.dump(childPids)

      -- only the supervisor will execute this part
      local alldone = 1
      repeat 
        alldone = 1
        for child_nr = 0, NUMBER_WORKERS do
           if childPids[child_nr] > 0 then
              wait_childpid, wait_status_string, wait_status_int = posix.wait(childPids[child_nr], posix.WNOHANG)
              if not (wait_childpid == nil or wait_childpid == 0)  -- must contains the pid of the process terminated
                  and (wait_status_string == "exited" 
                      or wait_status_string == "killed" 
                      or wait_status_string == "stopped") then
                 -- Child finished
                 print("Child " .. wait_childpid .. " " .. wait_status_string .. " due to: " .. wait_status_int)
                 childPids[child_nr] = 0;
              else
                 -- Child not finished
                 print("Child " .. childPids[child_nr] .. " unfinished")           
                 alldone = 0;
              end
           end
